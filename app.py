...
def generate_next_id(prefix, table, column):
    """Auto-generate next ID based on existing IDs"""
    conn = get_conn()
    result = conn.execute(
...
def show_metric_card(value, label, color=None):
    ...
    st.markdown(
        f'<div class="metric-card"><div class="metric-value" {style}>{value}</div><div class="metric-label">{label}</div></div>',
        unsafe_allow_html=True
    )
...
if page == "Dashboard":
    ...
    with c1:
        show_metric_card(total_penyewa, "Total Penyewa")
    with c2:
        show_metric_card(total_kamar, "Total Kamar")
    with c3:
        show_metric_card(kamar_tersedia, "Kamar Tersedia")
    with c4:
        show_metric_card(belum_lunas, "Belum Lunas", color="#e53e3e")
    with c5:
        show_metric_card(format_currency(total_pemasukan), "Total Pemasukan", color="#276749")
...
# ════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        with st.form("form_penyewa"):
            ...
            for i, (_, row) in enumerate(fasilitas_df.iterrows()):
                with fas_cols[i % 4]:
                    if st.checkbox(
                        f"{row['nama_fasilitas']} (+{format_currency(row['harga_tambahan'])})",
    ...
    with tab2:
        ...
        if df_p.empty:
            st.info("Tidak ada data penyewa.")
        else:
...
# ════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        if not existing_pb.empty:
            ...
            st.info(f"💡 Pembayaran terakhir: **{existing_pb.iloc[0]['id_pembayaran']}** | Kamar: **{last_kamar_id}** | Jumlah: **{format_currency(last_jumlah)}** | Status: **{existing_pb.iloc[0]['status_bayar']}**")
        else:
            last_kamar_id = None
        ...
        with st.form("form_bayar"):
            ...
            with c1:
                ...
                if kamar_terisi_df.empty:
                    ...
                    kamar_opts_terisi = [f"Kamar {r['no_kamar']} ({r['id_kamar']}) — {format_currency(r['harga_dasar'])}" for _, r in kamar_terisi_df.iterrows()]
            ...
            for i, (_, row) in enumerate(fasilitas_df.iterrows()):
                with fas_cols[i % 4]:
                    if st.checkbox(f"{row['nama_fasilitas']} (+{format_currency(row['harga_tambahan'])})", key=f"fas_{i}"):
                        fas_selected.append(row)
            ...
            st.info(f"💰 **Total Bayar: {format_currency(total)}**")
            ...
            if st.form_submit_button("💾 Simpan Pembayaran", use_container_width=True):
                if not id_kamar_val:
                    ...
                    try:
                        ...
                        conn.commit()
                        conn.close()
                        st.success(f"✅ Pembayaran **{new_id_bayar}** disimpan! Total: {format_currency(total)}")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Error: {e}")
    ...
    with tab2:
        ...
        if df_tagihan.empty:
            ...
            opts = [f"{r['nama']} — {r['id_pembayaran']} — {format_currency(r['jumlah_bayar'])}" for _, r in df_tagihan.iterrows()]
...
# ════════════════════════════════════════════════════════════════
    ...
    with c1:
        n = len(kamar_df)
        show_metric_card(n, "Total Kamar")
    with c2:
        n = len(kamar_df[kamar_df["status"] == "Tersedia"])
        show_metric_card(n, "Tersedia", color="#276749")
    with c3:
        n = len(kamar_df[kamar_df["status"] == "Terisi"])
        show_metric_card(n, "Terisi", color="#1a1a2e")
    with c4:
        n = len(kamar_df[kamar_df["status"] == "Maintenance"])
        show_metric_card(n, "Maintenance", color="#e53e3e")
    ...
    display_df["harga_dasar"] = display_df["harga_dasar"].apply(lambda x: format_currency(x))
