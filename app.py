...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        # Load kamar & fasilitas untuk form
        conn = get_conn()
        ...
        with st.form("form_penyewa"):
            ...
            c1, c2 = st.columns(2)
            with c1:
                auto_id_penyewa = generate_next_id(
    "P",
    "penyewa",
    "id_penyewa"
)
...
id_penyewa = st.text_input(
    ...
    value=auto_id_penyewa
           ...
           with c4:
    auto_id_bayar = generate_next_id(
    "PB",
    "pembayaran",
    "id_pembayaran"
)
...
id_bayar_awal = st.text_input(
    ...
    # 🔥 Tambahan fitur status pembayaran awal
    status_bayar_awal = st.selectbox(
        ...
        ["Belum Lunas", "Lunas"]
            ...
            st.info(
    f"💰 **Total Tagihan Awal: Rp{total:,}** "
    f"(Status: {status_bayar_awal})"
)
            ...
            if submitted:
                if not id_penyewa or not nama:
                    ...
                    try:
                        conn = get_conn()
                        # 1. Simpan data penyewa
                        conn.execute(
                            "INSERT OR IGNORE INTO penyewa VALUES (?,?,?,?,?,?)",
                            (id_penyewa, nama, alamat, no_hp, str(tgl_masuk), str(tgl_keluar))
                        )
                        # 2. Buat pembayaran awal otomatis — status Belum Lunas
                       # 2. Buat pembayaran awal otomatis
conn.execute(
    ...
    (
        ...
        status_bayar_awal
                        ...
                        # 3. Simpan fasilitas yang dipilih ke detail_kamar
